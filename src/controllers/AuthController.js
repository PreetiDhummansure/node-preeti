...
exports.login = async (req, res) => {
	try {
		...
		//added full name here
		const token = createToken({ name: `${user.fname} ${user.lname}${user.fullname}`, email: user.email });
		delete user.password;
		user.token = token;
		return success({ res, msg: "You are logged-in successfully.", data: user, status: 200 });
	} catch (err) {
